...
const EditButton = React.forwardRef(({ isEditing, ...props }, ref) => {
  return (
    <button
      ref={ref}
      {...props}
      className="relative flex items-center bg-[#EBEDFF] w-[78px] h-[31px] xl:w-[82px] xl:h-[35px] rounded-full px-2 overflow-hidden transition-all duration-300"
    >
      <motion.span
        initial={false}
        animate={{ x: isEditing ? 30 : 10 }}
        className="text-gray-700 font-medium text-[12px] xl:text-[13px] font-['Poppins'] transition-all duration-300"
      >
        {isEditing ? "Exit" : "Edit"}
      </motion.span>
      <motion.div
